import { createKsherClient } from "../src";

const appId = "mch35005";
...
describe("verifySignature", () => {
  test("exampleGatewaypayDataResponse", () => {
    const sdk = createKsherClient({ appId, privateKey });
    const exampleGatewaypayDataResponse = {
      code: 0,
      msg: "SUCCESS",
      data: {
        pay_content:
          "https://gateway.ksher.com/ua?order_uuid=f624f098b8d111edacef525400962f26&lang=en",
      },
  ...
  test("verifySignature_exampleGateway_order_queryDataResponse", () => {
    const sdk = createKsherClient({ appId, privateKey });
    const exampleGatewayOrderQueryDataResponse = {
      code: 0,
      msg: "操作成功",
      data: {
        channel: "",
    ...
    const verifySignature = sdk.verifySignature(
      exampleGatewayOrderQueryDataResponse
    );
    expect(verifySignature).toBe(true);
  });
  ...
  test("verifySignature_exampleNative_payDataResponse", () => {
    const sdk = createKsherClient({ appId, privateKey });
    const exampleNativePayDataResponse = {
      code: 0,
      status_code: "",
      status_msg: "",
    ...
    const verifySignature = sdk.verifySignature(exampleNativePayDataResponse);
    expect(verifySignature).toBe(true);
  });

  test("verifySignature_exampleRefund_queryDataResponse", () => {
    const sdk = createKsherClient({ appId, privateKey });
    const exampleRefundQueryDataResponse = {
      code: 0,
      data: {
        appid: "mch29217",
        cash_fee: "48",
    ...
    const verifySignature = sdk.verifySignature(exampleRefundQueryDataResponse);
    expect(verifySignature).toBe(true);
  });

  test("verifySignature_exampleMerchant_info_DataResponse", () => {
    const sdk = createKsherClient({ appId, privateKey });
    const exampleMerchantInfoDataResponse = {
      code: 0,
      data: {
        account_type: "Formal",
        agency_info: {
    ...
    const verifySignature = sdk.verifySignature(exampleMerchantInfoDataResponse);
    expect(verifySignature).toBe(true);
  });
});
