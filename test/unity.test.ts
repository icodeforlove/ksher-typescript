...
describe("utils", () => {
  ...
  test("convertDataToString", () => {
    ...
    const exampleGatewayCreateTextVerify =
    ...
    const textMakeSign = convertDataToString(exampleGatewayCreateData);
    expect(textMakeSign).toEqual(exampleGatewayCreateTextVerify);
  });

  test("isPrivateKeyPem", () => {
    const examplePath = "/ksher_pubkey.pem";
    const pathFormat = isPrivateKeyPem(examplePath);
...
-----END RSA PRIVATE KEY-----`;
    const privateKeyPem = isPrivateKeyPem(privateKey);
    expect(privateKeyPem).toEqual(true);
  });
});
